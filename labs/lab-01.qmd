...
**a.** Create a `my.date` parameter object set to the last day of water year 2023. Ensure it is stored as a proper `Date` object, not a character string.
...
::: callout-tip
`lag()` shifts a vector back by one position, so `Flow - lag(Flow)` gives today's flow minus yesterday's. Because you want to compute this *within* each gauge, `group_by(site_no)` first, then `arrange(Date)` to ensure chronological order before lagging.

Hint: structure this as: group by `site_no`, arrange by `Date`, then use `lag()` to compute `flow_delta` (so today's Flow minus yesterday's Flow).
:::
...
```{r}
# student: create two flextables for `my.date`
# e.g., table1 <- flow |> filter(Date == my.date) |> slice_max(Flow, n = 5)
# and then pass to `flextable()` with `set_header_labels()` and `autofit()`
```
...
::: callout-tip
Hint: compute a 7-day rolling mean (e.g., `roll7`) with `zoo::rollmean(..., k = 7, fill = NA, align = "right")` inside a `group_by(site_no)` + `arrange(Date)` block.
:::
...
::: callout-tip
Two steps: summarize baselines, then join them back. This is the same join pattern you practiced in Q2 — verify it the same way.

Hint: summarize the 10th-percentile threshold per gauge over the baseline period, then left-join those thresholds back to the full flow table so every row carries its gauge's threshold.
:::
...
::: callout-tip
`geom_ribbon()` draws a filled band between `ymin` and `ymax`. Make the band appear only during below-threshold periods:

Hint: use `geom_ribbon()` with `ymin` at the threshold and `ymax` set to `if_else(below_threshold, roll7, threshold)` so the ribbon only appears while the rolling mean is below threshold.
:::
...
**e.** How many days per year, on average, did Lees Ferry fall below its low-flow threshold in **2000–2010** vs. **2011–2023**? Produce a small summary table and answer in 2–3 sentences: (1) what direction did the exceedance frequency change? (2) Is this a shift in isolated dry years or a persistent baseline change? (3) What does this imply for Compact delivery obligations?
...
::: callout-tip
Hint: derive `month` from `Date` and set `water_year` so Oct–Dec map to the next calendar year (e.g., `if_else(month >= 10, year + 1, year)`).
...
::: callout-tip
Hint: create `season` with `case_when()` mapping month ranges to labels and convert to a factor with hydrological order: Snowmelt, Monsoon, Low-flow, Winter.
...
::: callout-tip
The dominant season join: filter your seasonal summary to Lees Ferry, then within each water year select the season with the highest total flow:
Hint: produce a single-row-per-year table by selecting Lees Ferry annual flows, left-joining Glenwood annual flows by `water_year`, joining the dominant season for Lees Ferry, and log-transforming the flow columns for modeling.

This is your fourth join. Verify: the final dataset should have ~44 rows, one per water year.
:::
