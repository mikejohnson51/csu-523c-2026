...
Three rules — every tidyverse tool assumes them:
...
`ggplot2` is built on the **Grammar of Graphics** — the idea that every chart can be described by the same small set of components:
...
Aesthetic mappings describe how variables in the data are **visualized**. Placed in `ggplot()`, they apply globally to all layers.
