...
| Packages | Install and load core packages before running examples | Install `tidyverse`, `sf`, `terra`, `leaflet`, `here` |
| Interactive Maps | Use `leaflet` for demos and labs | Run the `leaflet` setup chunk before map code |
...
| `leaflet` | Interactive web maps and map widgets (labs + demos) |
| `here` | Project-root-aware paths for reproducible file access |
| `broom` | Tidying model outputs for tabular summaries |
| `lubridate` | Date-time parsing and manipulation (time series) |
| `janitor` | Cleaning and inspecting tabular data (clean_names(), etc.) |
