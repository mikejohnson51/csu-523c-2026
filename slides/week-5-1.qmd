...
- So far we've fed a raw formula (`forested ~ .`) straight to a model.
...
#### 3. `Power / Variance-Stabilizing Transforms`

  - Reshape skewed distributions toward normality before scaling.

  - Less sensitive to outliers than min-max scaling.

  - Box-Cox (positive data) and Yeo-Johnson (positive or negative) estimate an optimal power transform from the data.
...
head(pred |> select(flipper_length_mm, .fitted, .resid))
```
:::
::: {.column width="55%"}
```{r example-plot}
ggscatter(pred,
          ...
          color = "species", palette = "jco") +
  labs(title = "Observed vs. Fitted: Flipper Length",
       x = "Observed (mm)", y = "Fitted (mm)")
```
:::
::::
...
:::{.callout-tip}
Swap `add_formula()` for `add_recipe(my_recipe)` and the same workflow now carries your feature engineering pipeline *and* the model together — `fit()` will `prep()`/`bake()` for you on every resample.
:::
...
On Wednesday, we'll talk more about models we can choose from...
